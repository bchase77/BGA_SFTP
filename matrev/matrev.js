 ...
 * MatRev implementation : © Mike & Jack McKeever and Bryan Chase bryanchase@yahoo.com
 *
...
function (dojo, declare) {
    return declare("bgagame.matrev", ebg.core.gamegui, {
        constructor: function(){
            console.log('matrev constructor');
        ...
        */
        ...
        setup: function( gamedatas )
        {
			...
			this.gamedatas = gamedatas; // Not sure why Liverpool Rummy doesn't need this line

			console.log("[bmc] Players object", this.gamedatas.players);

			if (!this.gamedatas.state || !this.gamedatas.state.name) {
				console.error("[bmc] Missing state name in gamedatas:", this.gamedatas);
			} else {
				console.log("[bmc] Initial state is:", this.gamedatas.state.name);
			}
            ...
            for( var player_id in this.gamedatas.players )
            {
				...
				var panel = this.getPlayerPanelElement( player_id );
				
				if ( panel ) {
					panel.insertAdjacentHTML('beforeend', `<div>Custom content for ${player.name}</div>`);
				} else {
					console.warn("[bmc] getPlayerPanelElement returned null for player", player_id);
				}
				...
				// MineBotTemp
				// MineConTemp	
				if( '' + this.gamedatas.playerOnOffense == player_id ){ 
				...
				if( '' + this.gamedatas.playerOnDefense == player_id ){ 
				...
				if( '' + this.gamedatas.playerOnTop == player_id ){ 
				...
				if( '' + this.gamedatas.playerOnBottom == player_id ){ 
